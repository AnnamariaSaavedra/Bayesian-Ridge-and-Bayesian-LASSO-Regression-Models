...
BETA_MEDIAN <- round(apply(M3$BETA, MARGIN = 2, FUN = median), 4) # Posterior median

BETA_SD <- round(apply(M3$BETA, MARGIN = 2, FUN = sd), 4) # Posterior standard deviation
...
SIGMA2_MEDIAN <- round(median(M3$SIGMA), 5) # Posterior median

SIGMA2_SD <- round(sd(M3$SIGMA), 5) # Posterior standard deviation
...
LAMBDA_MEDIAN <- round(median(M3$LAMBDA), 4) # Posterior median

LAMBDA_SD <- round(sd(M3$LAMBDA), 4) # Posterior standard deviation
...
# Overlay the posterior density estimate as a red line
