...
a <- c(rep(1602, 18), rep(102, 18), rep(3, 18)) # Shape parameter of inverse-gamma distribution

b <- c(rep(1601, 9), rep(16010, 9), rep(101, 9), rep(1010, 9), rep(2, 9), rep(20, 9)) # Scale parameter of inverse-gamma distribution

c <- c(rep(1600, 3), rep(100, 3), rep(1, 3)) # Shape parameter of gamma distribution

d <- c(1600, 160, 16, 100, 10, 1, 1, 0.1, 0.01) # Rate parameter of gamma distribution

hyperparameter <- cbind(a, b, rep(c, 6), rep(d, 6))

# Before executing the following code, sample_beta_ridge, sample_sigma2_ridge, and
# sample_lambda_ridge functions must be executed

# 4. Gibbs sampling algorithm

Gibbs_ridge <- function(y, x, hyperparameter, n, p, n_skip, n_sams, n_burn, verbose = TRUE)
{
  ...
  for (j in 1:nrow(hyperparameter)) {
    ...
    for (i in 1:B) {
      ...
      sigma2 <- sample_sigma2_ridge(beta, y, x, a = hyperparameter[j,1], b = hyperparameter[j,2], n) # Update sigma2
      lambda <- sample_lambda_ridge(beta, c = hyperparameter[j,3], d = hyperparameter[j,4], p)
...
M2 <- Gibbs_ridge(y, x, hyperparameter, n, p, 
...
simulation_ridge <- function(model, hyperparameter, p)
{
...
simulation_result <- simulation_ridge(M2, hyperparameter, p)
