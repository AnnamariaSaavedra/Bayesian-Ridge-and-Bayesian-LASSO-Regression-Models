...
sample_xi_lasso <- function(y, x, xi, beta, sigma2, alpha, e, f, g, h) {
  ...
  for (i in 1:n) {
    ...
    if (new_cluster == (K_new + 1)) {
      ...
      shape <- e + 0.5 # Shape parameter
      rate <- f + 0.5*(((y[i] - x_i%*%beta_prior)^2)) # Rate parameter
      
      new_sigma2 <- 1 / rgamma(1, shape = shape, rate = rate)
      
      # Full conditional distribution of beta
      Sigma <- solve((1/sigma2_prior)*(t(x_i)%*%x_i) + (diag(1/tau_prior, p)))
      mu <- Sigma%*%((1/sigma2_prior)*(t(x_i)%*%y[i]))
...
sample_beta_lassonp <- function(y, x, xi, sigma2, tau) 
{
  ...
  for (k in 1:K) {
    ...
    Z <- solve((1/sigma2[k])*(t(x_k)%*%x_k) + (diag(1/tau, p))) # Compute covariance matrix
...
sample_sigma2_lassonp <- function(y, x, xi, beta, e, f)
{
  ...
  for (k in 1:K) {
    ...
    shape <- (0.5*n_k) + e # Shape parameter
    rate <- (0.5*(sum((y_k - x_k%*%beta_k)^2))) + f # Rate parameter
...
Gibbs_lassonp <- function(y, x, n_burn, n_sams, n_skip, e, f, g, h, l, m, verbose = TRUE) {
  ...
  sigma2 <- 1/rgamma(K, shape = e, rate = f)
